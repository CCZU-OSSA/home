...
如果没有 Github 账户想要贡献此文章也欢迎直接进我们的 QQ 群来聊天：905914105

## 观前提醒

本篇文章的时间节点，如军训即为默认开学时军训，但实际军训可能会被移到10月，如果你在开学时没有军训，你可以把文中军训的时间节点当作开学。

## 警惕入学前/后常见谣言、骗局

### 1. 神秘勤工俭学/入学前必须加入QQ群聊

勤工俭学建议入学后咨询辅导员。

入学前需要加入的群聊基本只有学院群、班级群，新生群大多是由学生发起并且可能有多个（自行辨别）。

如果你加入的一个自称官方的群聊，请注意观察群名是否带 院校/专业/学校，群头像是否带校徽，都可以作为辨别依据。

### 2. 学长学姐的床垫被子等神秘床上用品

开学前，如果你在新生群可能会有自称学长、学姐的人来加你并推销。他们提供的床上用品不一定有保障，也有几率触发真结局直接送钱，不建议购买。

学校也有提供统一的预购收集表，不过我还是建议你自行购买，因为这样还能走售后。

在这推荐一下蓝盒子的床垫枕头，广受好评这一块。

### 3. 待补充

## 校园网、校园卡与一网通办

**注意：无论你办不办校园卡（即流量卡）都不会影响你的入学，请勿掉入营销陷阱！**

首先，校园网 ≠ 校园卡，校园卡通常指流量卡（另外还有个校园一卡通，通常指刷热水、洗澡的卡）。常大的 2 个校区均有**免费**校园网和开通校园卡（移动）后才能使用的专享校园网。各网络均设有不同账号密码。学校和移动有合作，宿舍的有线宽带也是移动的。校园卡的开通主要是图个流量多、方便，校园卡的套餐也是比正常套餐便宜的。校园卡最主要的好处是可以使用有线宽带，西太湖速度高达 500Mbps（62.5MB/s）（武进 9 号楼为 300Mbps，其余均为 100Mpbs，~~且无 ipv6~~ 于2024/8/28 发现武进10号楼移动宽带已支持ipv6，且为公网）我校的免费校园网速度也不差，在西太湖校区，免费校园网速度可达 ~~60Mbps（7.5MB/s）~~（2025 已限速至1MB/s 😅），专享校园网可达 80Mbps（10MB/s）（ 2 个月前的一场雨摧毁了专享校园网，至今未修理😅 ）
...
省流：鉴于校园网的难绷速度，建议每个宿舍都办一张卡，以便使用路由器提高宿舍网速。
...
西太湖校区则需要参照[常州大学认证计费软件操作手册](../pdf/常州大学认证计费软件操作手册.pdf)进行网络认证
...
专享校园网、有线宽带的账号为你开通的手机号，密码是不同的。如果你想使用校内移动营业厅开办的校园卡连接有线网或校园卡专属的CMCC无线网，而工作人员却没有告诉你有线密码应该如何是好？😕

- CMCC无线网只要在登陆页点一下 `获取密码` 就可以重置密码了（随机 6 位数字，无法自定义），这串数字密码并不同于传统的会自动失效的验证码，如果你愿意 ~~记得住~~，在获取一次密码后就可以一直将其作为登录密码使用了。

- 有线网（宽带）重置密码方法有两种，一是拨打 10086 咨询客服直接修改，这里更推荐第二种方式：发送短信选择重置密码。使用校园卡向 10086 发送 `359001` 或 `XGKDMM` 修改宽带密码；使用校园卡向 10086 发送 `359002` 或 `CZKDMM` 重置宽带密码；移动可能会向你的校园卡号码发送引导操作的短信，按要求回复身份证号或旧密码等即可。

- 向10086发送 `密码`，`密码重置`，`903` 等关键词都可以查询密码服务相关指令码，请务必确认你是在进行 **修改宽带密码** 而不是 修改密码。

### 替代宽带上网助手的选择

宽带上网助手本质上只是一个拨号工具，只要我们获得了拨号的账号，便可以直接进行拨号操作了。虽然你可能在别的地方使用过手机号作为账号登录，但是请记住， **手机号不等于拨号账户！**
...
转专业的机会一共有三次，分别在大一上下，大二上。

如果你需要转专业，建议大一上先过四级并且不挂科，因为这是转专业的参考依据之一，此外你还需要学习一下对应专业的基本知识，转专业分为笔试和面试，通常笔试通过第二天会面试，可以关注教务系统的状态更改。每年的专业转入人数有限制，可以关注一下教务处的网站以及学校的通知。如果还有疑问建议询问辅导员/班主任，因为他们也是你转专业的一环。

除了上面这些，不建议你ALLIN转专业，需要做好失败的打算（继续转or放弃）。
...
## 大学生应具备的基本能力
 ...
 - [计算机相关专业的冲浪指南](https://rainchan.win/SurfingTutorial/)
...
7 号楼上下铺，桌子上方有挡板挡视野，暴力拆它丫的（可能要在毕业的时候进行赔偿？）1层有独立卫浴，其它楼层有独卫无独浴，熄灯情况不知道。 10 号楼上下铺，无挡板，无独浴有独卫，10：30熄灯。每层都有洗衣机房。12，13号楼为新宿舍楼（苯环楼，外表神似苯环），有独卫、封闭式阳台、上下铺。
